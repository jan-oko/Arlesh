...
The presets judge each kind by its own rule: a Task and a Goal by their status and effective Archival, a Commitment by its Verdict, and an **Expectation** by its status and archive (see [*Expectations*](resources.md)). A **delegated Task** is judged as archived — it has every effect of archival — and a dependency on a pending Expectation makes a Task **blocked** — as does a dependency on a done Asynchronous Task whose **spawned** wait is still pending. A spawned wait is judged by the same Expectation rules as a stored one, reading its status and archive from the overlay and its Check every from the template, and "asynchronous" is read as *has an Expectation template*. The List View's **Expectations** option, like Unblock, is a flag beside the preset (`BoardFilter::expectations`) rather than a preset of its own, and the Mindmap ignores it.
