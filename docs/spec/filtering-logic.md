...
A Delegate is a Person or the Agent (see [*Tasks*](resources.md)), and both delegation fields treat the two alike: delegate-to offers the Agent as a value beside every Person, and a Task delegated to the Agent is **delegated**. Neither reads the Agentic flag, which is its own field.
