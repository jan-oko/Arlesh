...
It is a **plain boolean** (`asynchronous` — NOT NULL, default false), deliberately **not** the three-state inherited shape **Agentic** uses. The flag **does not inherit**, and with nothing to inherit there is no third state for an unset value to carry. The reason is the semantics, not the storage: "starts a wait" is a property of one concrete action, and a subtask of an asynchronous Task is usually the work you do *after* the wait — inheriting would flag exactly the wrong rows. **Tasks only**, on the narrower version of the Agentic argument: only a Task is *done*, and only doing it starts a wait. It is **independent of Blockers and Dependencies**, which model the wait itself — who or what is being waited on, and what cannot start until it ends; this flag says only that doing the Task begins one, and nothing about it is inferred from an edge.

Set from a **switch in the Task editor**, beside the Backlog switch rather than down in Advanced: it is a statement about the order the work wants to be done in, which is the same kind of question as whether the Task is set aside at all. It is also set from the bare **W** binding ("wait") in the Mindmap and List View, a plain two-state toggle of the Task's own flag — with no resolved value to read through, one press flips both the column and the badge. It shows as its own **status-row badge** (an hourglass, distinct from the Time Scope clock: a clock is a window that passes on its own, an hourglass is a wait somebody started) in both views, on the flagged Task alone and never on its children; and it is filterable as its own List View pill dimension (Asynchronous / Not asynchronous, with the usual Any/All/Exclusion modes). Retyping a Task to any other kind drops the flag and names it in the confirmation prompt alongside every other lost field; duplicating a Task copies it — the copy is the same action, so it starts the same wait. Sorting by it is a **List View** setting and opt-in; see [*List View*](list-view.md).
