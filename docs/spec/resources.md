...
- The **×** takes no confirmation. It is one nullable column, the issue is still in `bd` either way, and it is a user-sourced write, so the Undo Journal carries it and `Ctrl+Z` puts the link back — a dialog for that is friction. The row then **stays, greyed and without its ×, for the life of the editor** rather than vanishing: a dialog that reflows under the pointer hides the very thing it is reporting. Reopening the editor shows no row, which is the steady state.
