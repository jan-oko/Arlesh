...
describe("formatScope (single)", () => {
  it("renders a day as dd/mm/yy", () => {
    expect(formatScope(mk(1, "day", "2026-06-20"))).toBe("20/06/26");
  });
  it("renders a month by name with the year", () => {
    expect(formatScope(mk(1, "month", "2026-06-01"))).toBe("June 2026");
  });
  it("renders a season by name with the year", () => {
    expect(formatScope(mk(1, "season", "2026-06-01"))).toBe("Summer 2026");
  });
  it("renders a week as W{n} with the year", () => {
    expect(formatScope(mk(1, "week", "2026-06-14"))).toMatch(/^W\d+ 2026$/);
  });
});
...
describe("formatScopeRange", () => {
  it("collapses to a single scope when both ids match", () => {
    ...
    expect(formatScopeRange(scope, scope)).toBe("June 2026");
  });

  it("factors out a shared year for a month range", () => {
    expect(formatScopeRange(mk(1, "month", "2026-06-01"), mk(2, "month", "2026-08-01"))).toBe(
      "June-August 2026",
    );
  });

  it("keeps both years when a month range spans years", () => {
    expect(formatScopeRange(mk(1, "month", "2026-12-01"), mk(2, "month", "2027-01-01"))).toBe(
      "December 2026-January 2027",
    );
  });

  it("factors out a shared year for a day range (dd/mm-dd/mm YYYY)", () => {
    expect(formatScopeRange(mk(1, "day", "2026-06-28"), mk(2, "day", "2026-07-01"))).toBe(
      "28/06-01/07 2026",
    );
  });

  it("uses dd/mm/yy on both sides when a day range spans years", () => {
    expect(formatScopeRange(mk(1, "day", "2026-12-28"), mk(2, "day", "2027-01-05"))).toBe(
      "28/12/26-05/01/27",
    );
  });

  it("factors out a shared year for a week range", () => {
    expect(formatScopeRange(mk(1, "week", "2026-06-14"), mk(2, "week", "2026-07-12"))).toMatch(
      /^W\d+-W\d+ 2026$/,
    );
  });
});
