...
const tree = node("root", "domain", { children: [task, stored] });
...
describe("E on a wait — which editor opens", () => {
  it("opens the Expectation editor on a spawned wait, with no Plan from its Task", () => {
    render(<Harness />);
    ...
    expect(screen.getByLabelText("editor:fieldTitle")).toHaveValue("Waiting on the reply");
