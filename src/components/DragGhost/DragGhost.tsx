...
export default function DragGhost({ node, depth, x, y }: Props) {
  ...
  return createPortal(
    ...
    >
      ...
      <NodeIcon kind={node.kind} status={node.status} isBlocked={isBlocked} isHabit={node.flow?.isHabit === true} cx={iconCx} cy={height / 2} r={(iconWidth - 8) / 2} color={iconColor} opacity={iconOpacity} />
