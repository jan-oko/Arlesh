...
### Added
- Write-time scope-containment enforcement: a task's **Plan** must be wholly contained within its **Time Scope** (resolved on datetime boundaries); violating creates/updates are rejected with `TaskError::ScopeContainment`. Cross-tree `child ⊆ parent` enforcement and the parent-narrowing cascade prompt are not yet implemented
