...
### Added
- Narrowing a Task/Goal's Time Scope in the editor now detects nested items that would fall outside the new window and **prompts to clamp them to the new scope or cancel** (via `scope_containment_conflicts`), instead of a hard backend rejection
