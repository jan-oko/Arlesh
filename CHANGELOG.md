...
- **Complete a Habit iteration (Phase 8.4)**: clicking a virtual iteration node's status now toggles its completion — writing (or clearing) a `done` **Modification** for every flow item at that iteration's scope, with a `resolved_at` timestamp — so the next derivation classifies the iteration **Done** (and reverts to Active/Lapsed when un-checked). Backed by the new `set_habit_iteration_done` command. (Per-instance edits and dependency remaps remain for a follow-up.)
