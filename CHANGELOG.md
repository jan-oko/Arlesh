...
- **Redesigned filtering UI**: the status preset moved out of the popover into a compact **dropdown in the top bar** (gaining a 5th **Unblock** option while List View is active), and every active filter — tags plus, in List View, the eight new pill dimensions — now renders as a **chip in a top-bar row**, visible without opening anything. Click a chip's body to cycle Any → All → Exclude; its embedded **×** removes it. A chip's border/text color always encodes its mode (one fixed color per mode, shared across every dimension); its background additionally tints toward the value's aspect color where one is resolvable (tags, Parent, Antecedent, Dependency). The Filter popover itself is now a pure "add a filter" chooser — active filters never appear inside it — organized into a few always-expanded clusters ("Tags & Type", and, in List View, "Hierarchy" and "Status & Scope") instead of one long list behind a single "Advanced" collapse.
