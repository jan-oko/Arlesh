...
### Added
- **Flows (Phase 7)**: a Flow is a template node kind that materializes a Goal/Task subtree on demand. Create one via the **New Flow** context-menu action on an Aspect, Domain, Project, or Goal; it renders with a play-triangle icon and opens a **Flow editor** for its title, **Instance Type** (goal or task), Duration-form **flow scope** (N of day/week/month/season), and a **Target Node** search combobox. Backed by the `flows`/`flow_goals`/`flow_tasks` tables (migration `0008`) and `create_flow`/`get_flow`/`list_flows`/`update_flow`/`delete_flow` commands. Flow items, cycle scopes, and start-materialization land in later Phase 7 steps
