...
- **Per-item Habit instances (Phase 8.4)**: each iteration now expands into the flow's **items** as its own virtual, per-`(flow item, iteration scope)` instances (mirroring the template hierarchy), and each is **individually completable** — clicking one writes/clears a `done` **Modification** for just that item, and the iteration reads as **Done** once all its items are. Clicking the iteration root still toggles the whole thing at once. Item instances inherit the aspect colour and dim when their iteration has lapsed. Backed by the `list_habit_item_completions` and `set_habit_item_done` commands (alongside `set_habit_iteration_done`). (Per-instance field edits and dependency remaps remain for a follow-up.)
