...
### Changed
- **Achieved goals now read as a filled bullseye** instead of the hollow target icon, so a goal's completion is visible at a glance on the canvas. Since virtual Habit **goal instances** have no editor, clicking their icon toggles them **achieved** directly (goal instances read `achieved`/`active`, like a real goal, rather than `done`/`todo`).
