...
fn scope_containing_snaps_a_date_to_its_week() {
    ...
    assert_eq!(week.id.to_string(), "week:2026-09-20");
...
fn part_scope_names_the_band_on_its_day() {
    ...
    assert_eq!(part.id.to_string(), "part_of_day:2026-09-23:evening");
}
...
fn get_scope_derives_the_scope_a_key_names() {
    let key: ScopeKey = "month:2026-09-01".parse().unwrap();
...
fn resolve_scope_reports_iso_half_open_bounds() {
    let key: ScopeKey = "day:2026-06-20".parse().unwrap();
