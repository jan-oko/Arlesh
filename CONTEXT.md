...
**Agentic wait** — An Expectation an agent raised on the Agentic Task it is working: "the agent is waiting on you", the replacement for `bd human`. A boolean and an optional **note** on the Expectation; the note carries the agent's question, and the user answers by writing into it and releasing the wait. Only a direct child of a Task that reads as Agentic can be one.
