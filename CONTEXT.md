...
**Delegate** — Who holds a delegated Task: a **Person**, or the **Agent**. Stored as a `(kind, id)` pair on Tasks (`tasks.delegate_kind` / `delegate_id`) rather than a Person id, so delegating to an agent does not invent a Person; there is one Agent target and it carries no id. Inherits downward and is overridable, whichever kind either end is. Independent of **Agentic**: the flag says the work suits an agent, the Delegate says who holds it. Set to the Agent with the one-click **Delegate to agent** button beside the Agentic flag. Nothing about it dispatches anything.
