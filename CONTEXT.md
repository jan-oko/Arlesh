...
**Asynchronous** — A Task whose doing starts a **wait** rather than finishing something: send the email, order the part, kick off the build. It means the Task **has an Expectation template** (`task_async_templates`): title, tags, a Time Scope rule and Check every, and no status. Completing the Task **spawns** a virtual Expectation from it; un-completing retracts it; a Task depending on it stays blocked until that wait is released. It **does not inherit** — deliberately unlike **Agentic** — because "starts a wait" is a property of one concrete action, and a subtask of an asynchronous Task is usually the work you do *after* the wait. Tasks only. Edited in the Task editor and with `Shift+W`, toggled with bare `W` (default template "Waiting on {title}"), badged with an hourglass, filterable as its own List View pill dimension, and what the List View's opt-in **Asynchronous first** ordering reads. It was once a plain boolean flag; the template replaced it.
...
- Only a Task carries **Asynchronous** (an Expectation template), and it stops at the Task it is set on: a child of an asynchronous Task is not itself asynchronous. It is independent of every other flag on the Task.
