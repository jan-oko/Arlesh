...
**Expectation** — A wait: something outside your own action that you are waiting on to be released, and may want to monitor (a training run finishing, someone replying). A content node kind parented anywhere a Task can be, holding only Info notes. **Pending** until **Released**, and separately archivable. Tasks can depend on one, and a pending one blocks them; it depends on nothing. Carries a Time Scope and tags like a Task, but no Plan, and an optional **check-by**; while one is set, a virtual *check task* hangs beneath it. A delegated Task carries a virtual Expectation of its own, released when the Task is done. Not an action item: it is released, never done.
...
- Only a Task carries **Asynchronous**, and the flag stops at the Task it is set on: a child of an asynchronous Task is not itself asynchronous. It is independent of every other flag on the Task.
