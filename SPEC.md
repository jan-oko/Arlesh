...
- **"Status" cluster (Mindmap only)** — an **Archived** pill that cycles **Inactive → Include → Exclude → Inactive** on click, overriding the status preset's own handling of Archived-status Goals/Projects and scope-**Lapsed** items (the two states that render the status row's archive-box badge) independently of achieved/frozen, which stay governed by the preset alone. **Inactive** (default) reproduces today's per-preset behavior exactly — Plan/Start hide Archived/Lapsed as before, All shows them. **Include** force-shows Archived/Lapsed items even under Plan/Start. **Exclude** force-hides them even under All. Has no effect under **Do** (which already shows only in-progress tasks, never goals/containers, regardless of archived state).
