...
- `A` — cycle the selected Task's Agentic flag: **Inherit → Agentic → Not agentic → Inherit** (real Tasks only, on the same reasoning as `B`). Bare letters are flags on the selected Task, `Alt`+letter is a status preset, so `A` and `Alt+A` (the **All** preset) coexist exactly as `B` and `Alt+B` do — chord matching is strict about modifiers
