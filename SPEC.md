...
The subtree root is **shared between the Mindmap and the List View**, not per-view: entering a subtree in one and switching to the other leaves you in the same place, which is the same reasoning that makes the two views share a tree and a filter rather than drifting apart. The List View honours it by flattening from that node instead of the true root, so it lists exactly the Tasks beneath it, and its path headers start at the subtree root — the ancestors above are named by the top bar's back-nav pills and repeating them on every header is noise. Because the top bar holds no tree of its own, whichever view is mounted resolves the back-nav descriptor for it (`src/hooks/use-subtree-nav.ts`), so the pills are correct in both views rather than only the one that happens to own the subtree logic.
